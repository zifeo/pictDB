...
int do_create(const char* filename, struct pictdb_file* db_file) {
  ...
  // we initialize here all the other fields of the header that were not set
  // explicitly before
  ...
  if ((db_file->metadata = (struct pict_metadata*)calloc(
           db_file->header.num_files, sizeof(struct pict_metadata))) == NULL) {
  ...
  if (1 != fwrite(&db_file->header, sizeof(struct pictdb_header), 1,
                  db_file->fpdb) ||
      db_file->header.max_files !=
          fwrite(db_file->metadata, sizeof(struct pict_metadata),
